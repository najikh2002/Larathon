...
# ================================================================================
# File: routes/web_improved_example.py
# ================================================================================
"""
Example of improved routing system - Laravel style
This shows how routes SHOULD be defined with the new system
"""


def register_routes():
    """Register all routes with improved routing system"""
    ...
    Route.middleware(['auth']).group(lambda: [
        ...
        Route.prefix('storage').name('storage.').group(lambda: [
            Route.get('/', StorageController, 'index').name('index'),
            Route.post('/upload', StorageController, 'upload').name('upload'),
            Route.delete('/{path}', StorageController, 'delete').name('delete'),
        ])
    ])
    ...
    Route.prefix('api').name('api.').group(lambda: [
        Route.prefix('v1').name('v1.').group(lambda: [
            ...
            # Authenticated API
            Route.middleware(['auth:api']).group(lambda: [
...
url = route('admin.users.index')     # /admin/users
url = route('api.v1.user')           # /api/v1/user
...
def register_routes():
    ...
    Route.get("/posts/<post_id:int>/edit", PostController, "edit")
    Route.post("/posts/<post_id:int>", PostController, "update")
    Route.post("/posts/<post_id:int>/delete", PostController, "destroy")
    ...
    Route.delete("/storage/delete/<path>", StorageController, "delete")

# Test route for upload debugging
def test_upload_page(request):
    controller = Controller()
    return controller.view('storage.test', request)

Route.get("/storage/test", lambda request: test_upload_page(request))
